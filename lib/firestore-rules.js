...
service cloud.firestore {
  match /databases/{database}/documents {
    ...
    /** Plan free: aún no hay partidos fijados (solo admin podía escribir antes). */
    function canParticipantInitFreeMatchIds(groupId) {
      let g = get(/databases/$(database)/documents/groups/$(groupId));
      return g.data.planCode == 'free_3_matches' &&
             (g.data.freeMatchIds == null || g.data.freeMatchIds.size() == 0) &&
             g.data.maxMatchNumber != null &&
             g.data.maxMatchNumber > 0;
    }
    ...
    // Plan free: freeMatchIds.hasAny. Pagos: sin tope si no hay maxMatchNumber, o matchNumber <= tope.
    // Importante: un campo ausente no siempre cumple == null en reglas; usar keys().hasAny.
    function isWithinGroupMatchLimit(groupId, matchId) {
      ...
      return (groupDoc.data.planCode == 'free_3_matches' &&
             ...
             (groupDoc.data.planCode != 'free_3_matches' &&
              (!groupDoc.data.keys().hasAny(['maxMatchNumber']) ||
               groupDoc.data.maxMatchNumber == null ||
               groupDoc.data.maxMatchNumber == 0 ||
    ...
    match /groups/{groupId} {
      ...
      allow update: if isAuthenticated() && (
                      ...
                      (request.auth.uid != resource.data.adminUid &&
                       ...
                       !request.resource.data.diff(resource.data).affectedKeys().hasAny(['settings', 'adminUid', 'competitionId', 'name', 'code', 'isActive', 'createdAt', 'id', 'freeMatchIds'])) ||
                      // Plan free: cualquier participante puede fijar freeMatchIds una vez
                      (isGroupParticipant(groupId) &&
                       canParticipantInitFreeMatchIds(groupId) &&
                       request.resource.data.diff(resource.data).affectedKeys().hasOnly(['freeMatchIds', 'updatedAt']) &&
                       request.resource.data.freeMatchIds is list &&
                       request.resource.data.freeMatchIds.size() > 0 &&
                       request.resource.data.freeMatchIds.size() <= resource.data.maxMatchNumber)
                    );
